...
<body class="bg-gray-100">
	...
	<div class="container mx-auto mt-5">
		...
		<div class="bg-white shadow-sm rounded-lg p-4 mt-2">
			<p class="text-lg">
				Monto total solicitado (En revisión): <strong class="text-red-600">
					$ <%=montoTotalSolicitado != null ? new DecimalFormat("#,##0.00").format(montoTotalSolicitado) : "0.00"%>
				</strong>
			</p>
			<p class="text-lg">
				Monto total adjudicado (Vigente): <strong class="text-green-600">
			...
			<table
				...
				<tbody id="tableBody">
					...
					<tr>
						...
						<td class="border px-4 py-2 flex space-x-2"><a
							href="servletAprobarPrestamo?id=<%=prestamo.getId()%>"
							...
							onclick="return confirm('¿Estás seguro de que deseas aprobar este préstamo?');">
								<i
								class="fas fa-check transition-all duration-200 hover:text-blue-300"></i>
						</a> <a href="servletRechazarPrestamo?id=<%=prestamo.getId()%>"
							...
							onclick="return confirm('¿Estás seguro de que deseas rechazar este préstamo?');">
								<i
								class="fas fa-ban transition-all duration-200 hover:text-blue-300"></i>
						</a></td>
					</tr>
					<%
						}
	...
	function filterTable() {
		...
		for (let i = 1; i < rows.length; i++) {
			...
			if (cells[5]) { // Suponiendo que el Estado está en la sexta columna (índice 5)  
				...
				estadoFound = estadoText === filterEstado
						|| filterEstado === "";
			}
