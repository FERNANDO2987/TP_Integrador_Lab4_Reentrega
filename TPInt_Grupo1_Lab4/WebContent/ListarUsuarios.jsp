				...
				<tbody id="tableBody">
					...
					<tr>
						...
						<td class="px-4 py-2 border"><span
							class="badge <%=usuario.isAdmin() ? "bg-green-500 text-white" : "bg-red-500 text-white"%> px-2 py-1 rounded-full">
								<i
								class="<%=usuario.isAdmin() ? "fas fa-check" : "fas fa-times"%> mr-1"></i>
								<!-- Ícono de verificación o cruz --> <%=usuario.isAdmin() ? "Sí" : "No"%>
						</span></td>
						...
						<td class="px-4 py-2 border">
							<div class="relative inline-block text-left">
								...
								<button type="button"
									...
									id="dropdownButton<%=usuario.getId()%>">
									<i class="fas fa-ellipsis-v"></i>
									<!-- Icono de tres puntos -->
								</button>
								<!-- Menú desplegable oculto por defecto -->
								<div
									class="dropdown-menu absolute right-0 hidden bg-white border border-gray-200 rounded-md shadow-lg mt-1 z-10"
									id="dropdown<%=usuario.getId()%>">
									<!-- Opción para Modificar -->

									<!-- Opción para Eliminar -->
									<a href="servletEliminarUsuario?id=<%=usuario.getId()%>"
										class="dropdown-item flex items-center px-4 py-2 text-sm text-red-500 hover:bg-red-100"
